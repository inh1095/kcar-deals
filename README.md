...
**추천 5대 규칙**(`build_guide.pick_top5`, 상수 `PICK_SIZES`·`PICK_KM`·`PICK_N_HYB`·`PICK_HYB_MIN_ADAS`·`PICK_GRADES`):
어머님이 타시던 K5 크기 이상(중형·준대형 세단) · 7만km 이하만 대상으로 다섯 자리를 정해 뽑는다 —
가성비 1위(10년 총지출 최소) · 가심비 1위(만족 요소 최다) · 하이브리드 가심비 2대(안전장치 3+) ·
풀옵션 가성비 1위(장비 20+ · 안전 4종 · 건식 DCT 제외 · 안심/괜찮음). 같은 차종+연료 조합은 한 번만.
표는 8열(차·트림 / 연식 / 주행 / 차값 / 그 탭의 순위 기준 / 장비 / 안전 / 등급)만 보여주고 차 이름이 곧 링크다.
