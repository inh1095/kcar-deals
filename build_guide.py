#!/usr/bin/env python3
"""data/listings.csv → docs/index.html (어머님용 중고차 고르기 안내)

점수를 앞세우지 않는다. 매물마다 엔진·변속기·연료·주행거리·차급을 따져서
'무엇을 확인해야 하는지'를 문장으로 만들고, 확인할 점이 적은 순서로 보여준다.

    python3.11 build_guide.py

LLM 없이 단독 실행된다. 판정 규칙과 차종 지식은 car_knowledge.py 에 있다.
...
TOP_PICKS = 3
SECTION_N = 3
LIST_N = 40          # 전체 목록에 싣는 대수 (너무 길면 읽기 어렵다)
...
def load(csv_path="data/listings.csv"):
    ...
    for r in rows:
        for k in ("km", "price", "year", "cc", "group_n", "total_cost_est",
                  "group_median_km"):
...
def evaluate(r: dict) -> dict:
    ...
    warn: list[str] = []      # 경고 — 이것 때문에 포기할 수도 있는 사항
    caution: list[str] = []   # 주의 — 확인하면 넘어갈 수 있는 사항
    good: list[str] = []      # 좋은 점
    checks: list[str] = []    # 현장에서 확인할 것
    ...
    if eng["nature"] == "자연흡기" and pt["tx"] in (K.TX_AUTO, K.TX_IVT) and not eng["theta2"]:
        good.append("터보나 DCT 같은 복잡한 장치가 없는, 가장 단순하고 무난한 조합입니다")
    if eng["nature"] == "터보":
        caution.append("터보 엔진입니다")
        checks.append("터보 엔진은 시동을 걸고 바로 세게 밟지 않는 습관이 필요합니다. "
                      "시동 직후 '쉬익' 하는 큰 소리나 흰 연기가 나는지 보세요.")
    ...
    if pt["tx"] == K.TX_DCT_DRY:
        ...
        checks.append("건식 DCT는 가다 서다 하는 정체 구간에서 울컥거림이나 미세한 떨림이 "
                      "보고되는 방식입니다. **시승할 때 반드시 막히는 길이나 오르막에서 "
                      "시속 10~30km로 천천히 가보세요.** 울컥거림이 거슬리면 그 차는 "
                      "피하시는 게 낫습니다. 어머님이 주로 시내를 다니신다면 특히 중요합니다.")
    else:
    ...
    if r["km"] < 80000:
        good.append(f"주행거리가 {r['km']:,}km로 같은 조건 차들보다 짧습니다")
    ...
    pg = r["price_gap"]
    if pg is None:
        caution.append(f"비슷한 차가 {r['group_n']}대뿐이라 시세 비교가 어렵습니다")
    elif pg >= 0.10:
        good.append(f"같은 차·같은 연식 중앙값보다 {pg:.0%} 저렴합니다")
    elif pg <= -0.10:
        warn.append(f"같은 차·같은 연식 중앙값보다 {abs(pg):.0%} 비쌉니다")
    ...
    if "후방카메라" in r["opt"]:
        ...
        checks.append("후방카메라가 목록에 없습니다. 주차할 때 크게 도움이 되는 장치이니 "
                      "실제로 있는지 차에서 직접 확인하시고, 없으면 설치 비용(20~40만원)을 "
                      "감안하세요.")
    if "열선시트" in r["opt"]:
    ...
    # 세금·연료비 -----------------------------------------------------------
    van = K.is_van_registered(r["model"], r["trim"])
    tax = K.car_tax(r["cc"], r["year"], van=van)
    fuel_cost = K.annual_fuel_cost(pt["engine"], r["fuel"])
    if van:
        good.append("승합자동차로 등록된 차라 자동차세가 배기량과 무관한 정액입니다 "
                    "(같은 크기 승용차보다 훨씬 저렴합니다)")
        checks.append("9인승·11인승 카니발은 승합자동차로 등록돼 자동차세가 정액이고 "
                      "고속도로 버스전용차로를 쓸 수 있는 경우가 있습니다(6인 이상 탑승 시). "
                      "**자동차등록증의 '차종'이 승합인지 직접 확인하세요.** "
                      "다만 승합차는 매년 정기검사를 받아야 하는 점도 함께 보셔야 합니다.")
    elif r["cc"] and r["cc"] >= 2300:
    ...
    return dict(pt=pt, eng=eng, size=size, size_note=sn, fuel_note=fuel_note,
                ...
                grade=grade, tax=tax, fuel_cost=fuel_cost, order=order)


# ── 카테고리 추천 ────────────────────────────────────────────────────────────
def pick(items, pred, n=SECTION_N, exclude=(), one_per_model=True):
    """추천은 (1) 경고가 있는 차를 넣지 않고, (2) 같은 차종을 반복하지 않는다.

    같은 차종만 3대 늘어놓으면 고르는 데 도움이 되지 않으므로 차종당 1대만 싣는다.
    """
    ...
    for i in items:
        ...
        if one_per_model and key in seen_models:
            continue
        seen_models.add(key)
        out.append(i)
        if len(out) >= n:
...
def categories(items):
    top = pick(items, lambda i: i["ev"]["grade"] in ("안심", "괜찮음"), n=TOP_PICKS)
    return top, [
        ("조용하고 기름값이 가장 적게 드는 차",
         "시내와 정체 구간이 많으시면 하이브리드가 가장 편합니다. 저속에서 전기로만 "
         "움직여 소리가 거의 없고, 기름값은 일반 가솔린차의 절반 수준입니다.",
         pick(items, lambda i: i["fuel"] == "하이브리드", exclude=top)),
        ("타고 내리기가 가장 편한 차",
         "무릎이나 허리가 불편하시면 시트 높이가 중요합니다. 세단은 낮아서 앉을 때 "
         "허리를 굽히게 되고, 큰 SUV는 올라타야 합니다. 그 중간인 소형·준중형 SUV가 "
         "가장 편합니다.",
        ...
        ("짐과 손님이 많을 때 좋은 차",
         "손주를 태우거나 짐을 많이 실을 일이 잦으면 미니밴이나 중형 SUV가 편합니다. "
         "다만 차가 커서 주차는 각오하셔야 합니다.",
         pick(items, lambda i: i["ev"]["size"] in (K.SIZE_VAN, K.SIZE_LARGE_SUV), exclude=top)),
        ("돈을 가장 아끼는 차",
         "차값이 낮고 세금·기름값도 적게 드는 차입니다. 대신 주행거리가 길거나 "
         "수리 이력이 있을 수 있으니 확인 항목을 잘 보세요.",
...
FIELD_CHECKLIST = [
    ("가기 전에 집에서 (5분)", [
        ...
        "유료(약 2,200원)지만 침수·전손 여부까지 나오므로 꼭 본다.",
    ...
    ("차 밖에서 (10분)", [
        ...
        "문틈·보닛 안쪽에 붙은 <b>고무 몰딩을 살짝 들어</b> 도색이 겹쳐 있는지 본다.",
        "타이어 4개의 상표가 다 같은지, 옆면에 적힌 4자리 숫자(제조 주차)를 본다. "
        "예: '2419'는 2019년 24주 생산이다. 5년 넘었으면 교체 비용을 감안한다.",
    ...
    ("차 안에서 (10분)", [
        ...
        "브레이크 페달과 가속 페달의 고무 닳은 정도를 본다. 주행거리에 비해 많이 닳았으면 "
        "주행거리를 의심한다.",
        "에어컨을 가장 세게, 히터도 가장 뜨겁게 켜 본다. 찬바람·더운바람이 바로 나오는지, "
        "곰팡이 냄새가 나는지 본다.",
    ...
    ("시동과 엔진룸 (10분)", [
        "<b>가능하면 아침에, 시동을 한 번도 걸지 않은 차를 본다.</b> 미리 시동을 걸어 "
        "따뜻하게 해 둔 차는 냉간 시 증상을 감춘다.",
        ...
        "배터리 위에 붙은 제조일 스티커를 본다. 3년 넘었으면 곧 교체다(10~20만원).",
    ...
    ("서류와 계약 (10분)", [
        ...
        "K카 보증(기본 보증, 연장 보증) 범위와 기간을 문서로 확인한다. "
        "엔진·변속기가 포함되는지가 핵심이다.",
...
.greybox{background:#f2f1ee;border:1px solid #ddd;font-size:.9rem;color:#555}
...
.spec{display:flex;flex-wrap:wrap;gap:6px 0;margin:12px 0;padding:0;list-style:none;
...
.q{background:#fff;border:1px solid #ddd9d1;border-radius:12px;padding:16px 18px;margin:14px 0}
.q b{display:block;margin-bottom:8px}
.q label{display:block;padding:8px 10px;margin:4px 0;border:1px solid #e0ddd6;
border-radius:8px;cursor:pointer;font-size:.97rem;background:#fafafa}
...
def rich(s: str) -> str:
    """문장에서 <b> 와 **강조** 만 허용한다. 그 밖의 태그는 이스케이프한다."""
    out = html.escape(s).replace("&lt;b&gt;", "<b>").replace("&lt;/b&gt;", "</b>")
    parts = out.split("**")
    if len(parts) > 1:                      # **강조** → <b>강조</b>
        out = parts[0] + "".join(
...
<li>{h(ev['eng']['name'])}</li>
<li>{h(ev['pt']['tx'])}</li>
<li>{h(ev['size'])} · 주차 {h(ev['size_note']['park_level'])}</li>
<li>{h(r['location'])}</li>
</ul>
...
def build(rows, meta) -> str:
    ...
    dist_rows = "".join(
        ...
        f"<td>{GRADE_STYLE[g][1]}</td><td>{dist.get(g,0)}대</td></tr>"
    ...
    rest_rows = "".join(
        ...
        f"<td>{h(i['year_month'])}</td><td>{i['km']:,}</td><td>{i['price']:,}</td>"
...
<title>어머님을 위한 중고차 고르기 — K카 매물 정리</title>
<style>{CSS}</style>
</head><body><main>

<h1>어머님을 위한 중고차 고르기</h1>
<p class="lead">K카(kcar.com)에 올라온 국산 중고차를 조건에 맞게 걸러내고,
차종마다 <b>무엇을 확인해야 하는지</b>를 정리했습니다.
점수로 줄 세우지 않았습니다. 차마다 성격과 걸리는 점이 다르기 때문입니다.</p>

<div class="top">
<p><b>수집 일시</b> {h(meta.get('collected_at'))} · <b>출처</b> K카(kcar.com) 직영 매물</p>
<p><b>{meta.get('collected', 0):,}대</b>를 훑어 조건에 맞는 <b>{meta.get('passed', len(rows)):,}대</b>를
골랐고, 그중 확인할 점이 적은 차부터 보여드립니다.</p>
<p style="font-size:.88rem">{h(DISCLAIMER)}</p>
</div>

<div class="box tipbox">
<h3 style="margin-top:0">먼저 읽어주세요 — 이 정리의 한계</h3>
<p>이 페이지는 K카가 <b>매물 목록에 공개한 정보</b>만으로 만들었습니다.
K카 규칙상 각 차의 상세 페이지는 자동으로 열어보지 않았습니다. 그래서
<b>보험 수리 이력, 소유자가 몇 번 바뀌었는지, 정확한 총 구매비용, 보증 잔여 기간은
이 페이지에 없습니다.</b> 이 네 가지는 아래 '꼭 조회할 3곳'과 현장에서 직접
확인하셔야 합니다.</p>
<p>차종별 확인 사항은 <b>그 차종이 일반적으로 점검이 필요한 부분</b>을 적은 것입니다.
지금 이 차에 그 고장이 있다는 뜻이 아닙니다. 반대로, 여기 없는 문제가
있을 수도 있습니다. 그래서 <b>시승과 진단서 확인은 어떤 경우에도 생략하면 안 됩니다.</b></p>
</div>
...
집 근처 마트, 병원, 가까운 시내만 다니신다면 디젤은 배기 장치가 막혀
수리비가 크게 나올 수 있습니다. <b>편도 10km 안쪽을 주로 다니시면
가솔린이나 하이브리드를 고르세요.</b></p>
</div>

<h2>가장 먼저 보실 {len(top)}대</h2>
<p class="lead">확인할 점이 가장 적고, 복잡한 장치가 없어 오래 타기 편한 차들입니다.</p>
{top_html}

{''.join(cat_html)}

<h2>보러 가실 때 쓰는 체크리스트</h2>
<p class="lead">순서대로 하시면 됩니다. 이 페이지를 그대로 인쇄하시거나
휴대폰으로 열어두고 하나씩 확인하세요.
<b>아래 항목을 다 지키면 대부분의 나쁜 차는 걸러집니다.</b></p>
<p class="noprint"><button class="btn" onclick="window.print()">이 페이지 인쇄하기</button>
<a class="btn alt" href="#red">먼저 '이런 차는 사지 마세요'부터 보기</a></p>
{''.join(chk_html)}

<h2 id="red">이런 차는 사지 마세요</h2>
<p class="lead">아래 중 <b>하나라도</b> 해당되면, 값이 아무리 싸도 그 차는 넘어가세요.
...
'주의'라도 나쁜 차라는 뜻이 아니고, 확인할 것이 많다는 뜻입니다.
확인해서 문제가 없으면 좋은 차일 수 있습니다.</p>
<table class="simple" id="t-grade"><thead><tr><th>등급</th><th>뜻</th><th>대수</th></tr></thead>
<tbody>{dist_rows}</tbody></table>

<h2>나머지 후보 {len(rest)}대</h2>
<p class="lead">위에서 마음에 드는 차가 없을 때 보실 목록입니다.
각 차의 자세한 확인 사항은 K카 페이지에서 차를 확인하신 뒤,
위 '연료 고르기'와 '체크리스트'를 그대로 적용하시면 됩니다.</p>
<table class="simple" id="t-rest">
<thead><tr><th>차</th><th>연식</th><th>주행(km)</th><th>차값(만)</th>
...
<footer>
<p><b>출처</b> K카(kcar.com) 직영 매물 목록. 수집 {h(meta.get('collected_at'))}.
사진은 싣지 않았고, 각 매물의 K카 페이지로 연결만 합니다.</p>
<p><b>차종 정보의 근거</b> 엔진이 어느 차에 쓰였는지와 리콜이 있었는지는
공개된 엔진 자료(위키피디아 Hyundai Theta / R engine 문서)와
미국 집단소송 합의 공지에서 확인했습니다. 그 밖의 점검 항목은 널리 알려진
정비 상식을 정리한 것으로, <b>특정 차량에 결함이 있다는 뜻이 아닙니다.</b>
자동차세와 기름값은 이 페이지에서 계산한 <b>대략값</b>이며 실제 고지액·주행 습관에
따라 달라집니다.</p>
<p>{h(DISCLAIMER)}</p>
</footer>
</main></body></html>
