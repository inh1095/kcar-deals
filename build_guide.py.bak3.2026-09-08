...
COMMON_OK = 30
RARE = 15                # 이 미만이면 드문 차
MIN_ADAS_FOR_PICK = 2    # 추천 목록에 넣을 최소 첨단 안전장치 개수
...
def evaluate(r: dict, stats: dict) -> dict:
    ...
    # 8) 안전장치 — 어머님 조건에서 가장 중요한 축 -------------------------
    adas = r["adas_list"]
    n_adas, n_park, bags = r["adas_n"] or 0, r["parking_aid_n"] or 0, r["airbags"] or 0
    if n_adas >= 3:
        good.append(f"사고를 막아 주는 첨단 안전장치가 {n_adas}가지 있습니다 "
                    f"({', '.join(adas)})")
    elif n_adas == 0:
        caution.append("첨단 안전장치(자동 브레이크·차선 이탈 경고 등)가 확인되지 않습니다")
        checks.append("이 차는 <b>자동 긴급제동·차선이탈 경고·후측방 경보가 목록에 없습니다.</b> "
                      "같은 값대에 이런 장치가 있는 차가 많으니, 안전을 우선하신다면 "
                      "장치가 있는 차를 먼저 보시는 편이 낫습니다. "
                      "옵션 표기가 빠진 경우도 있으니 차에서 직접 확인하세요.")
    else:
        caution.append(f"첨단 안전장치가 {n_adas}가지뿐입니다")
    for name, why in (
            ("자동긴급제동", "앞차를 못 봤을 때 차가 스스로 브레이크를 잡아 줍니다. "
                          "안전장치 중 사고를 가장 많이 줄여 주는 기능입니다"),
            ("후측방경보", "차선을 바꿀 때 옆·뒤에서 오는 차를 사이드미러에 알려 줍니다"),
            ("차선이탈경보", "졸음이나 부주의로 차선을 넘으면 경고해 줍니다"),
            ("스마트크루즈", "앞차와 거리를 자동으로 맞춰 줍니다. 장거리 운전이 훨씬 편합니다")):
        if name in adas:
            good.append(f"{name} — {why}")
    if bags >= 4:
        good.append(f"에어백이 {bags}종류(운전석·조수석·사이드·커튼) 들어 있습니다")
    elif bags <= 2:
        caution.append(f"에어백이 {bags}종류만 확인됩니다")
    ...
    if n_park >= 3:
        good.append(f"주차 보조 장치가 {n_park}가지 있습니다")
    if "열선시트" in r["opt"]:
        good.append("열선시트(엉덩이 따뜻해지는 기능)가 있습니다")
    if "통풍시트" in r["opt"]:
        good.append("통풍시트(여름에 시원한 기능)가 있습니다")
    if "스마트키" in r["opt"]:
    ...
    order -= n_adas * 1.2               # 첨단 안전장치를 크게 우대
    order -= min(n_park, 4) * 0.3
...
def pick(items, pred, n=SECTION_N, exclude=(), one_per_model=True):
    ...
    out = []
    for i in items:
        # 안전을 앞에 두는 페이지이므로 첨단 안전장치가 2가지 미만인 차는 추천하지 않는다.
        if (i["adas_n"] or 0) < MIN_ADAS_FOR_PICK:
            continue
...
def categories(items):
    ...
    return top, [
        ("안전장치가 가장 많은 차",
         "사고를 막아 주는 장치가 가장 많이 달린 차입니다. 자동 긴급제동(앞차를 못 봤을 때 "
         "차가 스스로 브레이크를 잡는 기능)은 안전장치 중에서도 사고를 가장 많이 줄여 줍니다. "
         "2020년 이후 차는 이런 장치가 기본으로 들어가는 경우가 많습니다.",
         pick(items, lambda i: (i["adas_n"] or 0) >= 4 and (i["parking_aid_n"] or 0) >= 3
              and i["accident"] == "무사고")),
...
CHANNELS = [
    ("K카 (케이카) 직영", "https://www.kcar.com/bc/search",
     "이 페이지가 쓰는 곳입니다. 회사가 직접 사서 진단하고 파는 '직영'이라 "
     "차량 상태 기준이 일정하고, 일정 기간 책임 환불 제도가 있습니다. "
     "홈서비스로 집까지 배송도 됩니다.", "수집함", True),
    ("현대·제네시스 인증중고차", "https://certified.hyundai.com/p/search/vehicle",
     "<b>안전과 보증만 보면 여기가 가장 낫습니다.</b> 제조사가 직접 정밀 진단하고 "
     "신차 보증의 남은 기간을 이어 주며, 별도 보증을 더 붙일 수도 있습니다. "
     "현대·기아·제네시스만 있고 값은 같은 조건에서 다른 곳보다 비쌉니다. "
     "5년/10만km 이내 차량만 취급합니다.", "수집 못 함(사이트가 자동 조회를 거부)", False),
    ("기아 인증중고차", "https://www.kia.com/kr/customer-service/certified-used-car",
     "현대 인증중고차와 같은 구조의 기아 공식 채널입니다. "
     "제조사 진단·보증이 붙습니다.", "미조사", False),
    ("엔카 (encar)", "https://www.encar.com/dc/dc_carsearchlist.do",
     "매물이 가장 많은 곳입니다. 다만 대부분 <b>딜러가 올린 매물</b>이라 "
     "차량 상태와 응대 품질이 딜러마다 다릅니다. '엔카 진단'이 붙은 매물만 골라 보시고, "
     "허위·미끼 매물을 걸러야 합니다.", "수집 금지(robots.txt가 매물 데이터를 금지)", False),
    ("KB차차차", "https://www.kbchachacha.com/public/search/main.kbc",
     "KB캐피탈이 운영하는 딜러 매물 플랫폼입니다. 엔카와 성격이 비슷하며 "
     "금융(할부) 조건을 함께 보기 편합니다.", "미수집(목록이 자동 조회로 안 열림)", False),
]
...
def card(r: dict, rank: int | None = None) -> str:
    ...
    if r["price_gap"] is None:
        ...
        gap = (f'같은 차·같은 연식·<b>같은 트림</b> {r["group_n"]}대의 중앙값 '
               f'{r["group_median_price"]:,.0f}만원 대비 <b>{r["price_gap"]:+.0%}</b>'
               f'{" (저렴)" if r["price_gap"] > 0 else " (비쌈)" if r["price_gap"] < 0 else ""}')
    else:
        gap = (f'<span style="color:#8a5a12">{basis} {r["group_n"]}대 기준으로는 '
               f'{r["price_gap"]:+.0%}지만, <b>트림이 섞여 있어 이 숫자는 믿을 수 없습니다</b></span>')
    return f"""
<div class="card">
...
<li>안전장치 <b>{r['adas_n'] or 0}가지</b> · 에어백 {r['airbags'] or 0}종 · 주차보조 {r['parking_aid_n'] or 0}가지</li>
<li>{h(r['location'])}</li>
</ul>
...
def build(rows, meta, stats) -> str:
    ...
    channels = "".join(
        f"<h3>{h(n)}{' — 이 페이지가 쓰는 곳' if used else ''}</h3>"
        f"<p>{rich(d)}</p>"
...
<p class="lead"><b>2020년 이후 · 5인승 · 안전장치 위주</b>로 다시 골랐습니다.
2020년 이후 차는 자동 긴급제동·차선이탈 경고 같은 안전장치가 기본으로 들어가는 경우가 많고,
에어백도 4종류가 표준입니다.</p>
...
<p>이 중 <b>첨단 안전장치를 4가지 다 갖춘 차가 {n_adas4}대</b>,
에어백 4종류를 갖춘 차가 {n_bag4}대입니다.</p>
...
<h2>어디서 사는 게 좋은가</h2>
<p class="lead">중고차는 <b>어디서 사느냐가 차를 고르는 것만큼 중요합니다.</b>
같은 차라도 파는 곳에 따라 진단 기준, 보증, 환불 조건이 크게 다릅니다.
아래는 실제로 확인한 내용입니다.</p>
...
신차 보증 잔여 기간을 이어 주기 때문에, 큰 고장이 났을 때 내 돈이 나가지 않습니다.
그다음이 <b>K카 같은 직영</b>(회사가 직접 사서 파니 기준이 일정하고 환불 제도가 있음),
마지막이 <b>딜러 매물 플랫폼</b>(엔카·KB차차차 — 매물은 많지만 딜러마다 다름)입니다.</p>
<p><b>이 페이지에 실린 차는 전부 K카 직영 매물입니다.</b> 엔카는 사이트 규칙(robots.txt)이
